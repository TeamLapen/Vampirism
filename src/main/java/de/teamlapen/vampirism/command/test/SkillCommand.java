    ...
    private static int skill(CommandSource commandSource, ServerPlayerEntity asPlayer, ISkill skill) {
        ...
        switch (result) {
            case OK:
                ...
                commandSource.sendFeedback(new TranslationTextComponent("command.vampirism.test.skill.enabled", skill.getRegistryName() + " (" + new TranslationTextComponent(skill.getTranslationKey()).getFormattedText() + ")"), true);
                return 0;

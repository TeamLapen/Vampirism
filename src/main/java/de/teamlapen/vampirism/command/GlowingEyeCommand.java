...
public class GlowingEyeCommand extends BasicCommand {
    ...
    private static int setGlowingEye(@NotNull CommandContext<CommandSourceStack> context, @NotNull Player player, boolean on) {
        ...
        if (on) {
            context.getSource().sendSuccess(() -> Component.translatable("command.vampirism.base.glowing_eyes.enabled"), false);
        } else {
            context.getSource().sendSuccess(() -> Component.translatable("command.vampirism.base.glowing_eyes.disabled"), false);
        }
        return 0;
    }
}

    ...
    public ActionResultType onItemUse(ItemUseContext ctx) {
        ...
        if (player == null || player.canPlayerEdit(pos, ctx.getFace(), stack) && player.canPlayerEdit(otherPos, ctx.getFace(), stack)) {
            ...
            if (flag1 && flag2 && UtilLib.doesBlockHaveSolidTopSurface(world, pos.down()) && UtilLib.doesBlockHaveSolidTopSurface(world, otherPos.down())) {
                BlockState state1 = ModBlocks.med_chair.getDefaultState().with(BlockMedChair.PART, BlockMedChair.EnumPart.BOTTOM).with(BlockMedChair.FACING, facing.getOpposite());
                if (world.setBlockState(pos, state1, 3)) {
                    BlockState state2 = state1.with(BlockMedChair.PART, BlockMedChair.EnumPart.TOP).with(BlockMedChair.FACING, facing.getOpposite());

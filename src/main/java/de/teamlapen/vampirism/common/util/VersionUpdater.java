...
public class VersionUpdater {
    ...
    private static void update1_9to1_10() {
        ...
        ModConfig.BALANCE.skillPointsPerLevel.set(skillPointsPerLevel);
        double skillPointsPerLordLevel = ModConfig.BALANCE.skillPointsPerLordLevel.get();
        if (skillPointsPerLordLevel == 1) {
            skillPointsPerLordLevel = ModConfig.BALANCE.skillPointsPerLordLevel.getDefault();
        } else {
            skillPointsPerLordLevel *= 2;
        }
        ModConfig.BALANCE.skillPointsPerLordLevel.set(skillPointsPerLordLevel);
    }
}

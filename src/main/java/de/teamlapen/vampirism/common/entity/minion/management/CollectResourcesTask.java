...
import static de.teamlapen.vampirism.common.entity.minion.management.CollectResourcesTask.Desc;
        ...
        if (--desc.coolDown <= 0) {
            ...
            if (!stacks.isEmpty()) {
                ...
                if (stack.get(DataComponents.REPAIRABLE) != null && stack.getDamageValue() > 0) {
                    stack.setDamageValue(Math.max(0, stack.getDamageValue() - ModConfig.BALANCE.miEquipmentRepairAmount.get()));
                }
            }
        }
    }

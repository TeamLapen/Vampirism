...
import de.teamlapen.vampirism.common.network.packets.client.ClientboundTaskPacket;
import de.teamlapen.vampirism.common.network.packets.client.ClientboundTaskStatusPacket;
import de.teamlapen.vampirism.common.network.packets.server.ServerboundTaskActionPacket;
...
public class TaskManager<T extends ITaskPlayer<T>> implements ITaskManager, IDefaultSavable {
    ...
    public int getTaskTimeConfig() {
        if (ServerLifecycleHooks.getCurrentServer().isDedicatedServer()) {
            return ModConfig.BALANCE.taskDurationDedicatedServer.get();
        }
        return ModConfig.BALANCE.taskDurationSinglePlayer.get();
    }
    ...
    private @NotNull Collection<ITaskInstance> getTasks(@NotNull UUID taskBoardId) {
        ...
        wrapper.taskAmount = wrapper.taskAmount < 0 ? player.getRandom().nextInt(ModConfig.BALANCE.taskMasterMaxTaskAmount.get()) + 1 - wrapper.lessTasks : wrapper.taskAmount;
    ...
    public static class TaskWrapper {
        ...
        public static final Codec<TaskWrapper> CODEC = RecordCodecBuilder.create((instance) -> instance.group(
                ModCodecs.UUID.fieldOf("id").forGetter(i -> i.id),
                Codec.INT.fieldOf("lessTasks").forGetter(i -> i.lessTasks),
                Codec.INT.fieldOf("taskAmount").forGetter(i -> i.taskAmount),
                Codec.unboundedMap(ModCodecs.UUID, TaskInstance.CODEC).fieldOf("tasksSize").forGetter(i -> (Map<UUID, TaskInstance>) (Object) i.tasks),

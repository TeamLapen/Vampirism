...
import de.teamlapen.vampirism.world.old.gen.features.config.VampireDungeonConfig;
import de.teamlapen.vampirism.world.old.loot.LootHandler;
...
public class VampireDungeonFeatureOLD extends Feature<VampireDungeonConfig> {TODO 1.14 structures  wait for forge to add pieces to existing structures

    private static final Logger LOGGER = LogManager.getLogger(VampireDungeonFeatureOLD.class);
        ...
        if (airSpaces >= 1 && airSpaces <= 5) {
                    ...
                    if (worldIn.isAirBlock(blockpos2)) {
                        ...
                        if (solidSides == 1) {
                            ...
                            if (tileentity1 instanceof ChestTileEntity) {
                                ...
                                LOGGER.warn("Failed to generate dungeon chest at ({})", ModWorld.debug ? blockpos2 : "hidden");
                            }
                ...
                for (int j = 0; j < 3; ++j) {
                    ...
                    if (worldIn.isAirBlock(blockpos2)) {
                        ...
                        if (solidSides == 2) {
                            ...
                            if (tileentity1 instanceof BloodContainerTileEntity) {
                                ((BloodContainerTileEntity) tileentity1).setFluidStack(new FluidStack(ModFluids.blood, BloodBottleFluidHandler.getAdjustedAmount((int) (BloodContainerTileEntity.CAPACITY * rand.nextFloat()))));
                            } else {
                                LOGGER.warn("Failed to generate blood container in dungeon at ({})", ModWorld.debug ? blockpos2 : "hidden");
                            }
            ...
            if (tileentity instanceof AltarInspirationTileEntity) {
                tileentity.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY).ifPresent(handler -> {
                    handler.fill(new FluidStack(ModFluids.blood, (int) (AltarInspirationTileEntity.CAPACITY * rand.nextFloat())), true);
                });
                VampirismWorldData.get(worldIn.getWorld()).addNewVampireDungeon(position);

            } else {
                LOGGER.warn("Failed to generate altar of inspiration in dungeon at ({})", ModWorld.debug ? position : "hidden");
            }


            if (ModWorld.debug) LOGGER.info("Generated vampire dungeon at {}", position);
            return true;
        } else {
            return false;
        }

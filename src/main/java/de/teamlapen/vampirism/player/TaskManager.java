...
public class TaskManager implements ITaskManager {
    ...
    @Nonnull
    public static Set<Task> getTasks(PlayerEntity player, Function<? super ITaskManager, Set<Task>> mapper) {
    ...
    public void removeRequirements(@Nonnull Task task) {
        if (task.getRequirement().getType().equals(TaskRequirement.Type.ITEMS)) {
            this.player.inventory.func_234564_a_(itemStack -> itemStack.getItem() == task.getRequirement().getStat(), task.getRequirement().getAmount(),player.container.func_234641_j_() /*Not sure if this is correct*/);
        }
    }

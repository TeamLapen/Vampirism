...
public class ActionManager implements IActionManager {
    ...
    public <T extends IFactionPlayer<T>> List<IAction<T>> getActionsForFaction(IPlayableFaction<T> faction) {
        return RegUtil.values(VampirismRegistries.ACTIONS).stream().filter(action -> action.getFaction() == faction).map(action -> (IAction<T>)action).collect(Collectors.toList());
    }

    @Override
    public IForgeRegistry<IAction<?>> getRegistry() {
        return VampirismRegistries.ACTIONS.get();
    }
}

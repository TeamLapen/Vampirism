...
public class JEIPotionMix {
    public static Collection<JEIPotionMix> createFromMix(ExtendedPotionMix mix) {
        List<ItemStack> in1 = mix.reagent1.map(Ingredient::getItems).map(Arrays::stream).orElseGet(Stream::empty).map(ItemStack::copy).peek(s -> s.setCount(mix.reagent1Count)).filter(s -> !s.isEmpty()).collect(Collectors.toList());
        List<ItemStack> in2 = mix.reagent2.map(Ingredient::getItems).map(Arrays::stream).orElseGet(Stream::empty).map(ItemStack::copy).peek(s -> s.setCount(mix.reagent2Count)).filter(s -> !s.isEmpty()).collect(Collectors.toList());
        List<JEIPotionMix> recipes = new ArrayList<>(3);
        recipes.add(build(mix, Items.POTION, mix.input.get(), mix.output.get(), in1, in2));
        if (mix.output.get().getEffects().stream().noneMatch(s -> s.getEffect().getCategory() != EffectType.HARMFUL)) {

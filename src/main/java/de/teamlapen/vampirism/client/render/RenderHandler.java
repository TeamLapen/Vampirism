...
import de.teamlapen.vampirism.entity.player.vampire.actions.BatAction;
...
public class RenderHandler {
    ...
    public void onRenderHand(RenderHandEvent event) {
        if (VampirePlayer.get(mc.thePlayer).getActionHandler().isActionActive(ActionHandler.batAction)) {
            event.setCanceled(true);
        }
    }
    ...
    public void renderTick(TickEvent.RenderTickEvent event) {
        ...
        if (mc.theWorld != null) {
            if (event.phase == TickEvent.Phase.START) {
                if (VampirePlayer.get(mc.thePlayer).getActionHandler().isActionActive(ActionHandler.batAction) && mc.gameSettings.thirdPersonView == 0) {

                    //batTransform_ySize = -(float)mc.thePlayer.getYOffset() + BatAction.BAT_EYE_HEIGHT ;
...
//                        mc.thePlayer.posY -= batTransform_ySize;
                    mc.thePlayer.eyeHeight = mc.thePlayer.getDefaultEyeHeight() - BatAction.BAT_EYE_HEIGHT + (float) mc.thePlayer.getYOffset();

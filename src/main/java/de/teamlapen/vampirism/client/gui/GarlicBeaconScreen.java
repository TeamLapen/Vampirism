...
public class GarlicBeaconScreen extends Screen {
    ...
    protected void init() {
        ...
        startupBar = this.addRenderableWidget(new ProgressBar(this, this.guiLeft + (xSize - 170) / 2, this.guiTop + 30, 170, new TranslatableComponent("gui.vampirism.garlic_beacon.startup")));
        startupBar.setColor(0xD0D0FF);
        startupBar.setFGColor(0xFFFFFF);

        fueledTimer = this.addRenderableWidget(new ProgressBar(this, this.guiLeft + (xSize - 170) / 2, this.guiTop + 60, 170, new TranslatableComponent("gui.vampirism.garlic_beacon.fueled")));

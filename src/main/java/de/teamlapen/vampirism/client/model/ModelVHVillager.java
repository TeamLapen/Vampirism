 ...
 * @author Moritz
 *
 *         A villager that has become a vampire hunter. Similar model to the villagers, but it has arms like a regular player which can hold an item
 */
public class ModelVHVillager extends ModelVillager {
	...
	public void setRotationAngles(float f1, float f2, float f3, float f4, float f5, float f6, Entity e) {
		...
		this.rightVillagerLeg.rotateAngleX = MathHelper.cos(f1 * 0.6662F) * 1.4F * f2 * 0.5F;
		this.leftVillagerLeg.rotateAngleX = MathHelper.cos(f1 * 0.6662F + (float) Math.PI) * 1.4F * f2 * 0.5F;
		this.rightVillagerLeg.rotateAngleY = 0.0F;
		this.leftVillagerLeg.rotateAngleY = 0.0F;

		this.rightArm.rotateAngleX = MathHelper.cos(f1 * 0.6662F) * 1.4F * f2 * 0.5F;
		this.leftArm.rotateAngleX = MathHelper.cos(f1 * 0.6662F + (float) Math.PI) * 1.4F * f2 * 0.5F;
	}

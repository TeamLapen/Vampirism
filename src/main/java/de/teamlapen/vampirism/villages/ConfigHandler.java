...
/**
 * Reads the VillageBiomes.cfg (this is not a standard config file)
 * Has methods to retrieve specific config lines based on codes at the beginning of each line
 * Will copy VillageBiomesDefault.cfg from resources to the config folder as VillageBiomes.cfg if it doesn't already exist
 * @author WILLIAM
 *
 */
    ...
    public static Map<Block, List<Pair<String, Block>>> getReplacements() {
        ...
        for (String line : configLines) {
            ...
            if (m.matches()) {
                ...
                Logger.i("ConfigHandler", "Will replace " + b.getUnlocalizedName() + " with " + replacement.getUnlocalizedName() + " where " + condition);

...
public class EntityAIAttackVillage<T extends EntityVampirism & IVillageCaptureEntity> extends EntityAITarget {
    ...
    public boolean shouldExecute() {
        ...
        List<EntityLivingBase> list = this.attacker.world.getEntitiesWithinAABB(EntityLivingBase.class, attacker.getTargetVillageArea(), VampirismAPI.factionRegistry().getPredicate(attacker.getFaction(), false));
        if (list.isEmpty()) {
            return false;
        } else {

...
public interface TaskMasterEntity extends ForceLookEntityGoal.TaskOwner, ITaskMasterEntity {
    ...
    default boolean processInteraction(PlayerEntity playerEntity, Entity entity) {
        if (FactionPlayerHandler.getOpt(playerEntity).map(FactionPlayerHandler::getCurrentFactionPlayer).filter(Optional::isPresent).map(Optional::get).map(IFactionPlayer::getTaskManager).map(taskManager -> {
            return taskManager.hasAvailableTasks(entity.getEntityId());
        }).orElse(false)) {
            OptionalInt containerIdOpt = playerEntity.openContainer(new SimpleNamedContainerProvider((containerId, playerInventory, player) -> new TaskMasterContainer(containerId, playerInventory), CONTAINER_NAME.deepCopy()));

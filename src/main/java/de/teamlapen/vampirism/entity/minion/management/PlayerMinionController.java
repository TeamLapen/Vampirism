...
public class PlayerMinionController implements INBTSerializable<CompoundNBT> {
    ...
    public void markDeadAndReleaseMinionSlot(int id, int token) {
        ...
        if (i != null) {
            ...
            getLord().ifPresent(player -> {
                player.getLordFaction().getPlayerCapability(player.getPlayer()).map(ISkillPlayer::getSkillHandler).ifPresent(s -> {
                    if(ModRegistries.SKILLS.getValues().stream().filter(MinionRecoverySkill.class::isInstance).anyMatch(s::isSkillEnabled)){
                        i.deathCooldown *= 0.8;
                    }
                });
            });
    ...
    private LazyOptional<? extends ILordPlayer> getLord() {
        return getLordPlayer().map(FactionPlayerHandler::getOpt).orElseGet(LazyOptional::empty);
    }

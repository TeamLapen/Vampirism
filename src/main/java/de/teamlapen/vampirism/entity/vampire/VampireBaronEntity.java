...
//        }
    ...
    public void setLevel(int level) {
        if (level >= 0) {
            ...
            this.setCustomName(this.getType().getName().shallowCopy().appendSibling(new TranslationTextComponent("entity.vampirism.vampire_baron.level", level + 1)));
        } else {
    ...
    public void livingTick() {
        ...
        if (!this.world.isRemote && this.getAttackTarget() != null && this.ticksExisted % 128 == 0) {
            if (rangedAttack) {
                if (this.rand.nextInt(2) == 0 && this.navigator.func_75494_a(this.getAttackTarget(), 0) != null) {                //getPathToEntityLiving
                    rangedAttack = false;
                }
            } else {

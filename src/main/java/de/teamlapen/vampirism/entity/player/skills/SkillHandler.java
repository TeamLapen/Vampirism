...
public class SkillHandler<T extends IFactionPlayer> implements ISkillHandler<T> {
    ...
    public boolean canSkillBeEnabled(ISkill skill) {
        if (isSkillEnabled(skill)) return false;
        ...
        if (node != null) {
            if (node.isRoot() || isNodeEnabled(node.getParent())) {
                return !isNodeEnabled(node);//If another skill in that node is already enabled this one cannotbe enabled
            }
        }
        return false;
    }
    ...
    @Override
    public boolean isNodeEnabled(SkillNode skill) {

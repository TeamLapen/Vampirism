...
//    }
    ...
    public void handleUpdateTag(@Nonnull CompoundNBT tag) {
        ...
        if (tag.contains("village_bb")) {
            ...
            ModEventFactory.fireUpdateBoundingBoxEvent(bb);
    ...
    public void initiateCapture(@Nonnull IPlayableFaction faction, PlayerEntity player) {
        ...
        this.markDirty();
        if (ModEventFactory.fireInitiateCaptureEvent(world, controllingFaction, capturingFaction)) {
            ...
            if (!world.isRemote && capturingFaction == VReference.VAMPIRE_FACTION) {
                for (VillagerEntity v : villager) {
                    if (v instanceof FactionVillagerEntity)
                        continue;
    ...
    public void markDirty() {
        super.markDirty();
        world.notifyBlockUpdate(getPos(), world.getBlockState(pos), world.getBlockState(pos), 3);
        MutableBoundingBox bb = new MutableBoundingBox(UtilLib.bbToInt(getAffectedArea()));
        ModEventFactory.fireUpdateBoundingBoxEvent(bb);
    ...
    @Override
    public void remove() {
//        VampirismVillage v = getTotemTile();
//        if (v != null) {
...
//                    VampirismVillage village = this.getTotemTile();
//                    if (village != null) {
...
//                            int max = (int) Math.min(village.getTotemTile().getNumVillageDoors() * 1.5f, 30);
//                            if (l.size() < max) {
...
//
//                            }
//                            int defenderNumMax = Math.min(6, village.getTotemTile().getNumVillageDoors() / 5);
...
//            }
    ...
    public void updateTotem() {
        ...
        if (isComplete) {
            //@Nullable VampirismVillage village = getTotemTile();
            boolean insideVillageNew = false;//village != null;
...
//    @Nullable
//    private VampirismVillage getTotemTile() {
...
//    }
    ...
    private ResourceLocation getEntityForFaction(@Nonnull IFaction f) {
        ...
        return ModEventFactory.fireSpawnCaptureEntityEvent(f);
    }
    ...
    private boolean spawnEntityInVillage(@Nonnull Entity newEntity, @Nullable Entity entityToReplace) {
        if (entityToReplace != null) {
            ...
            BlockPos pos = BlockPos.ZERO;//findRandomSpawnPos(village.getTotemTile(), (MobEntity) newEntity);
    ...
    private boolean spawnVillagerInVillage(VillagerEntity newVillager, @Nullable Entity entityToReplace, boolean poisonousBlood) {
        ...
        if (entityToReplace instanceof VillagerEntity) {
            ...
            //VampirismVillage village = this.getTotemTile();
            //if (village == null) return false;
            //newVillager.setHomePosAndDistance(village.getTotemTile().getCenter(), village.getTotemTile().getVillageRadius());
        }
...
//        @Nullable VampirismVillage v = getTotemTile();
//        AxisAlignedBB box;
//        BlockPos b = v == null ? this.pos : v.getTotemTile().getCenter();
//        int r = v == null ? 30 : v.getTotemTile().getVillageRadius() + 15;
...
//        }
    ...
    private void updateCreaturesOnCapture() {
        List<VillagerEntity> villager = this.world.getEntitiesWithinAABB(VillagerEntity.class, getAffectedArea());
        //if(getVillage() != null) {
        if (ModEventFactory.fireVillagerCaptureEvent(villager, controllingFaction, capturingFaction, getAffectedArea()))
            return;

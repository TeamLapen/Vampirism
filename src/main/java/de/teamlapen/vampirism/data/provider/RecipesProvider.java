...
public class RecipesProvider extends RecipeProvider {
    ...
    protected void buildRecipes(@NotNull RecipeOutput output) {
        ...
        ShapedWeaponTableRecipeBuilder.shapedWeaponTable(RecipeCategory.COMBAT, ItemDataUtils.createEnchantment(ModItems.HUNTER_AXE_ENHANCED.get(), Enchantments.KNOCKBACK, 3)).lava(5).skills(HunterSkills.MASTER_CRAFTSMANSHIP.get()).pattern("XWZY").pattern("XWZY").pattern("  ZY").pattern("  Z ").define('X', iron_ingot).unlockedBy("has_iron", has(iron_ingot)).define('Y', garlic).define('W', diamond).define('Z', stick).save(output);

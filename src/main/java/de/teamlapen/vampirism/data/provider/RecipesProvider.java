...
public class RecipesProvider extends RecipeProvider {
    ...
    protected void buildRecipes(@NotNull RecipeOutput output) {
        ...
        AlchemicalCauldronRecipeBuilder.cauldronRecipe(ModItems.GARLIC_DIFFUSER_CORE_IMPROVED.get()).withIngredient(garlic_diffuser_core).withFluid(holy_water_bottle_ultimate).withSkills(HunterSkills.GARLIC_DIFFUSER_IMPROVED).experience(2.0f).save(output, modId("garlic_diffuser" +
                "_core_improved"));
        ...
        ShapedWeaponTableRecipeBuilder.shapedWeaponTable(RecipeCategory.COMBAT, ItemDataUtils.createEnchantment(ModItems.HUNTER_AXE_ENHANCED.get(), Enchantments.KNOCKBACK, 3)).lava(5).skills(HunterSkills.MASTER_CRAFTSMANSHIP).pattern("XWZY").pattern("XWZY").pattern("  ZY").pattern("  Z ").define('X', iron_ingot).unlockedBy("has_iron", has(iron_ingot)).define('Y', garlic).define('W', diamond).define('Z', stick).save(output);

...
public class MinionContainer extends InventoryContainer {
    ...
    public static MinionContainer create(int id, PlayerInventory playerInventory, MinionEntity<?> minionEntity) {
        Optional<Inventory> minionInv = minionEntity.getInventory();
        return minionInv.map(inv -> new MinionContainer(id, playerInventory, IWorldPosCallable.of(minionEntity.world, minionEntity.getPosition()), inv, createSelectors(minionEntity, inv.getSizeInventory()))).orElse(null);
    }

    private static SelectorInfo[] createSelectors(MinionEntity<?> minionEntity, int slotCount) {
        ...
        SelectorInfo[] slots = new SelectorInfo[slotCount];
        slots[0] = new SelectorInfo(factionPredicate.and(stack -> stack.canEquip(EquipmentSlotType.MAINHAND, minionEntity)), 1, 1, 1);
        slots[1] = new SelectorInfo(factionPredicate.and(stack -> stack.canEquip(EquipmentSlotType.HEAD, minionEntity)), 1, 1, 1);
        slots[2] = new SelectorInfo(factionPredicate.and(stack -> stack.canEquip(EquipmentSlotType.CHEST, minionEntity)), 1, 1, 1);
        slots[3] = new SelectorInfo(factionPredicate.and(stack -> stack.canEquip(EquipmentSlotType.LEGS, minionEntity)), 1, 1, 1);
        slots[4] = new SelectorInfo(factionPredicate.and(stack -> stack.canEquip(EquipmentSlotType.FEET, minionEntity)), 1, 1, 1);
        slots[5] = new SelectorInfo(Predicates.alwaysFalse(), 1, 1, 1);
        int extraSlots = slotCount - 6;
        assert extraSlots == 9 || extraSlots == 16 || extraSlots == 25 : "Minion inventory has unexpected size";
        for (int i = 0; i < extraSlots; i++) {
            slots[5 + i] = new SelectorInfo(Predicates.alwaysTrue(), 1, 1);
        }

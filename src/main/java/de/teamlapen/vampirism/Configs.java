...
public class Configs {
	...
	private static void loadConfiguration() {
		...
		village_density = config.get(cat_village.getQualifiedName(), "density", 10, "Minecraft will try to generate 1 village per NxN chunk area. Vanilla: 32").getInt();
		village_minDist = config.get(cat_village.getQualifiedName(), "minimumDistance", 4, "Village centers will be at least N chunks apart. Must be smaller than density. Vanilla: 8").getInt();
		village_size = config.get(cat_village.getQualifiedName(), "size", 0, "A higher size increases the overall spawn weight of buildings.").getInt();
	 ...
	 * To use this, the given class can only contain static non-final variables, which should have a '@Default*' annotation containing the default value.
	...
	private static void loadFields(ConfigCategory cat,Class cls){
		for(Field f:cls.getDeclaredFields()){
			...
			try {
				if(type==int.class){
					DefaultInt a=(DefaultInt)f.getAnnotation(DefaultInt.class); //Possible exception should not be caught so you cant forget a default value
					f.set(null,config.get(cat.getQualifiedName(), name, a.value(),a.comment()).getInt());
				}
				else if(type==double.class){
					DefaultDouble a=(DefaultDouble)f.getAnnotation(DefaultDouble.class); //Possible exception should not be caught so you cant forget a default value
					f.set(null,config.get(cat.getQualifiedName(), name, a.value(),a.comment()).getDouble());
				}
				else if(type==boolean.class){
					DefaultBoolean a=(DefaultBoolean)f.getAnnotation(DefaultBoolean.class); //Possible exception should not be caught so you cant forget a default value

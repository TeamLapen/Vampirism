...
public class DifficultyCalculator {
	...
	private static Difficulty calculateDifficulty(List<EntityPlayer> list) {
		...
		return new Difficulty(min, max, Math.round(sum / list.size()));
	}
	...
	public static Difficulty getLocalDifficulty(World w, double cX, double cZ, int r) {
		List<EntityPlayer> list = w.getEntitiesWithinAABB(EntityPlayer.class, AxisAlignedBB.getBoundingBox(cX - r, 0, cZ - r, cX + r, w.getActualHeight(), cZ + r));

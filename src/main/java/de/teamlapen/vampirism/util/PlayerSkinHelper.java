...
public class PlayerSkinHelper {
    ...
    public static void updateGameProfileAsync(final GameProfile input, Consumer<GameProfile> callback) {
        if (input != null && !StringUtils.isNullOrEmpty(input.getName())) {
            ...
            } else if (TileEntitySkull.profileCache != null && TileEntitySkull.sessionService != null) {
                THREAD_POOL.submit(() -> {
                    GameProfile gameprofile = TileEntitySkull.profileCache.getGameProfileForUsername(input.getName()); //This might create race conditions with other game profile updates. Maybe this has to be moved to the main thread

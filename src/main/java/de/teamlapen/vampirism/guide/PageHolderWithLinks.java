...
import de.teamlapen.vampirism.client.gui.GuiLinkedEntry;
        ...
        if (mouseX > guiEntry.guiLeft + guiEntry.xSize) {
            for (int i = 0; i < links.size(); i++) {
                if (GuiHelper.isMouseBetween(mouseX, mouseY, guiEntry.guiLeft + guiEntry.xSize, guiEntry.guiTop + 10 + 20 * i, links.get(i).width, 20)) {
                    Minecraft.getMinecraft().displayGuiScreen(new GuiLinkedEntry(book, categoryAbstract, links.get(i).entry, entityPlayer, guiEntry.bookStack, entryAbstract, guiEntry.pageNumber));
                    return;
                }
            }
        }

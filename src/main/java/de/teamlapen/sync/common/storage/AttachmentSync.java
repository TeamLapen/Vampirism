...
public abstract class AttachmentSync<T extends IAttachment, Z extends IAttachmentHolder> implements IAttachmentSerializer<T>, AttachmentSyncHandler<T>, Function<IAttachmentHolder, T> {
    ...
    public void write(RegistryFriendlyByteBuf buf, T attachment, boolean initialSync) {
        try (var problemReported = new ProblemReporter.ScopedCollector(attachment.asEntity().problemPath(), LOGGER)) {
            ...
            attachment.serializeUpdate(output, UpdateParams.all());
    ...
    public static abstract class PlayerOptions<T extends IAttachment> extends AttachmentSync<T, Player> {
        @Override
    ...
    public static abstract class PathFinderMobOptions<T extends IAttachment> extends AttachmentSync<T, Mob> {
        @Override
        public T apply(IAttachmentHolder holder) {

...
export const ItemImg = ({src, alt, imageHeight = 200}) => (
    <img src={src} alt={alt} style={{height: (typeof imageHeight === 'number' ? `${imageHeight}px` : imageHeight), imageRendering: "pixelated"}}/>
);

export const Items = ({children, title, img, source, subHeading = false, imageHeight = 100}) => {

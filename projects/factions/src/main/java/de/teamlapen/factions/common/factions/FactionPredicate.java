        ...
        switch (input) {
            ...
            case Player player1 when player && input.isAlive() -> {
                return FactionPlayerHandler.getCurrentFactionPlayer(player1).map(fp -> {
                            ...
                            return (IFaction.isNeutral(f) && (IFaction.is(sourceFaction, FactionTags.HOSTILE_TOWARDS_NEUTRAL) && neutral)) && !sourceFaction.equals(f) && (targetFaction.isEmpty() || (IFaction.is(targetFaction.get(), f)));
                        }
                ).orElse(neutral);
            }
            default -> {

...
public class VampirismMenu extends InventoryContainerMenu implements ITaskMenu {
    ...
    public VampirismMenu(int id, @NotNull Inventory playerInventory) {
        super(FactionMenus.VAMPIRISM.get(), id, playerInventory, ContainerLevelAccess.NULL, new SimpleContainer(3), RemovingSelectorSlot::new, SELECTOR_INFOS.apply(playerInventory.player));
        ...
        this.registry = playerInventory.player.level().registryAccess().lookupOrThrow(FactionRegistries.Keys.TASK);
    }
    ...
    public @NotNull TaskAction buttonAction(@NotNull ITaskInstance taskInfo) {
        return taskInfo.isUnique(this.registry) || this.factionPlayer.asEntity().level().getGameTime() < taskInfo.getTaskTimeStamp() ? ITaskMenu.TaskAction.ABORT : TaskAction.REMOVE;
    }

...
public class TaskEntryWidget extends AbstractWidget {
    ...
    private static final WidgetSprites ACCEPT_BUTTON = new WidgetSprites(
            ...
            FResourceLocation.mod("widget/task_action_accept_highlighted")
    );
    private static final WidgetSprites COMPLETE_BUTTON = new WidgetSprites(
            FResourceLocation.mod("widget/task_action_complete"),
            FResourceLocation.mod("widget/task_action_complete_highlighted")
    );
    private static final WidgetSprites ABORT_BUTTON = new WidgetSprites(
            FResourceLocation.mod("widget/task_action_abort"),
            FResourceLocation.mod("widget/task_action_abort_highlighted")
    );
    private static final WidgetSprites LOCATE_TASK_MASTER = new WidgetSprites(
            FResourceLocation.mod("widget/locate_task_master"),
            FResourceLocation.mod("widget/locate_task_master_highlighted")
    );
    ...
    private void sendCoordinateMessage() {
        ...
        if (lastKnownPosition != null) {
            ...
            message = Component.translatable("gui.factions.faction_menu.last_known_pos").append(itextcomponent.append(Component.translatable("gui.factions.faction_menu.distance", i)));
        } else {
            message = Component.translatable("gui.factions.faction_menu.last_known_pos.unknown").withStyle(ChatFormatting.GOLD);
        }
    ...
    private void buildWidgets() {
        ...
        if (menu.showLocateTaskmaster()) {
            ...
            Component message;
            if (lastKnownPosition != null) {
                message = Component.translatable("gui.factions.faction_menu.last_known_pos").append(Component.literal("[" + lastKnownPosition.toShortString() + "]").withStyle(ChatFormatting.GREEN));
            } else {
                message = Component.translatable("gui.factions.faction_menu.last_known_pos.unknown").withStyle(ChatFormatting.GOLD);
            }
    ...
    private void renderTaskTooltip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        if (menu.isTaskNotAccepted(taskInstance)) {
            tooltip.add(Component.translatable("gui.factions.taskmaster.not_accepted").withStyle(ChatFormatting.ITALIC, ChatFormatting.GRAY));
        } else {
    ...
    private Component getTimerComponent() {
        ...
        if (remainingTime >= 0) {
            ...
            return Component.translatable("gui.factions.taskmaster.task_failed").withStyle(ChatFormatting.BLACK);
        }
    }

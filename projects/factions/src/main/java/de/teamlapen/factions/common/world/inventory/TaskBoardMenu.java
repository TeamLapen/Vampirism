...
public class TaskBoardMenu extends AbstractContainerMenu implements ITaskMenu {
    ...
    public TaskBoardMenu(int id, @NotNull Inventory playerInventory) {
        super(FactionMenus.TASK_MASTER.get(), id);
    ...
    public boolean areRequirementsCompleted(@NotNull ITaskInstance task, @NotNull TaskRequirement.Type type) {
        ...
        if (this.completedRequirements != null) {
            if (this.completedRequirements.containsKey(task.getId())) {
                for (TaskRequirement.Requirement<?> requirement : getTask(task.getTask()).requirements().requirements().get(type)) {
    ...
    public boolean stillValid(@NotNull Player playerIn) {
        return !IFaction.isNeutral(FactionPlayerHandler.get(playerIn).getFaction());
    }

...
import de.teamlapen.factions.api.factions.skills.*;
...
import de.teamlapen.factions.common.util.collections.LiveMap;
...
public class SkillHandler<T extends IFactionPlayer<T> & ISkillPlayer<T>> extends PropertySync implements ISkillHandler<T> {
    ...
    protected void registerProperties() {
        ...
        this.registerProperty(FResourceLocation.mod("skills")).simple(SafeCast.<Codec<Map<Holder<ISkillTree>, List<Holder<ISkill<T>>>>>>cast(ENABLED_SKILLS_CODEC)).defaultValue(HashMap::new).provider(() -> this.enabledSkills).commonLoader(map -> {

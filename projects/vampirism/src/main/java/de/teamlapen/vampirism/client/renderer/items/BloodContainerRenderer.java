...
public class BloodContainerRenderer implements SpecialModelRenderer<SimpleFluidContent> {
    ...
    @Override
    public void submit(@org.jspecify.annotations.Nullable SimpleFluidContent argument, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, int overlayCoords, boolean hasFoil, int outlineColor) {
    ...
    public record Unbaked() implements SpecialModelRenderer.Unbaked<SimpleFluidContent> {
        ...
        @Override
        public BloodContainerRenderer bake(BakingContext context) {
            return new BloodContainerRenderer();
        }

        @Override
        public MapCodec<? extends SpecialModelRenderer.Unbaked<SimpleFluidContent>> type() {
            return MAP_CODEC;
        }
    }

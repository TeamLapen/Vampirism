...
public class MarshallCommand extends BasicCommand {

    public static ArgumentBuilder<CommandSourceStack, ?> register() {
        return Commands.literal(ModFactions.HUNTER.getKey().identifier().toString())
                .then(Commands.literal("marshall")
    ...
    public static int setDracula(CommandContext<CommandSourceStack> context, Collection<ServerPlayer> players) {
        for (ServerPlayer player : players) {
            ...
            if (handler.setFaction(FactionUpdate.maxLord(ModFactions.HUNTER).add(new IMarshallPlayer.MarshallChange()))) {
                context.getSource().sendSuccess(() -> Component.translatable("command.vampirism.base.marshall.success", player.getDisplayName()), true);
            } else {

...
public class AlchemicalCauldronScreen extends AbstractContainerScreen<AlchemicalCauldronMenu> implements IMinecraftAccessor {
    ...
    public AlchemicalCauldronScreen(AlchemicalCauldronMenu inventorySlotsIn, Inventory inventoryPlayer, Component name) {
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        ...
        this.menu.checkRecipeNoSkills().ifPresent(holder -> {
            boolean allSkills = HunterPlayer.get(player()).getSkillHandler().areSkillsEnabled(holder.value().getRequiredSkills());
            if (!allSkills) {
    ...
    protected void extractTooltip(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        if (mouseX > i + 77 && mouseX < i + 77 + 28 && mouseY > j + 32 && mouseY < j + 32 + 21) {
            this.menu.checkRecipeNoSkills().ifPresent(holder -> {
                List<Holder<? extends ISkill<?>>> missingSkills = holder.value().getRequiredSkills().stream().filter(s -> !HunterPlayer.get(player()).getSkillHandler().isSkillEnabled(s)).toList();
    ...
    protected void extractLabels(GuiGraphicsExtractor graphics, int xm, int ym) {
        Component name = Component.translatable("container.vampirism.alchemical_cauldron.owner", player().getDisplayName().copy().withStyle(ChatFormatting.DARK_BLUE), ModBlocks.ALCHEMICAL_CAULDRON.get().getName());

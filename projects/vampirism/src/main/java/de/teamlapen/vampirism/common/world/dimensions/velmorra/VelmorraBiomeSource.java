...
public class VelmorraBiomeSource extends BiomeSource {
    public static final MapCodec<VelmorraBiomeSource> CODEC = RecordCodecBuilder.mapCodec(inst -> {
        return inst.group(
                        RegistryOps.retrieveElement(ModBiomes.VELMORRA))
                .apply(inst, inst.stable(VelmorraBiomeSource::new));
    });

    private final Holder<Biome> biome;

    public VelmorraBiomeSource(HolderGetter<Biome> biomeGetter) {
        this.biome = biomeGetter.getOrThrow(ModBiomes.VELMORRA);
    }

    private VelmorraBiomeSource(Holder<Biome> biome) {
        this.biome = biome;
    }
